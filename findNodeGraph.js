...
class GNode {
  ...
  //using dfs
   // Write your code here.
      //
      //check for cycles by tracking visited nodes
      //trverse every node
      //at each node, check if the target value matches
      //if so, return true
       // repeat the same check for children
       //if there no children screenLeft, then the algo determined has no paths to target value
 function hasPathTo(target) {
     ...
     function dfs(node,target){
        ...
        for(const child of node.children){
            if(!visited.has(child) && hasPathTo(child,target)){
                return true
            }
            return false
        }
    ...
    return dfs(this,target)
  }

//bfs
function hasBFSPathTo(target){
